...
int NumberBlocksX = cellPerWidth_MAX / cellPerBlock;
int NumberBlocksY = cellPerHeight_MAX / cellPerBlock;
...
float cell_hog(int *magnitude,int *orientation,int start, int stop,int oStart, int oEnd,int r_index, int c_index)
{
    ...
    for (int y = start; y < stop; y++)
    {
        ...
        if (cri >= 0 && cri < MAX_HEIGHT)
        {
            ...
            for (int x = start; x < stop; x++)
            {
                cci = c_index + x;
                bool c1 = cci < 0;
                bool c2 = cci >= MAX_WIDTH;
                bool c3 = orientation[cri * MAX_WIDTH + cci] >= oStart;
                bool c4 = orientation[cri * MAX_WIDTH + cci] < oEnd;
                if (c1 || c2 || c3 || c4)
                {
                }
                else
                {
                    total += magnitude[cri * MAX_WIDTH + cci];
                }
            }
        }
    }
...
void normalizeCell(float *cellValues)
{
	int len = 36;//4 Block times 9 angular bins
    //if (mode == "L1"){
	float sum = 0;
	for (int i = 0; i < len; i++)
	{
		sum += hls::abs(cellValues[i]);
	}
	if (sum != 0){
		for (int i = 0; i < len; i++)
		{
...
void hog_picture_old(int16_t *Gx, int16_t *Gy, int nrOrientation,float scale,obj *objectList){

    static int magnitude[MAX_HEIGHT*MAX_WIDTH];
    static int orientaion[MAX_HEIGHT*MAX_WIDTH];
    static float histogram[cellPerHeight_MAX][cellPerWidth_MAX][angles];
    static float Blockhistogram[NumberBlocksY_MAX][NumberBlocksX_MAX][angles*4];
    int step = anglesMax / nrOrientation;

    for (int y = 0; y < MAX_HEIGHT; y++){
        for (int x = 0; x < MAX_WIDTH; x++){
            int pos   = y * MAX_WIDTH + x;
            int alphaMOD = MODULO((int)(hls::atan2f(Gy[pos],Gx[pos])*(float)anglesMax/3.1415926f),anglesMax);
            int root = hls::sqrt((int32_t)(Gx[pos]*Gx[pos]+Gy[pos]*Gy[pos]));
            magnitude[pos]  = root;
            orientaion[pos] = alphaMOD;
        }
    }
	...
	for (int i = 0; i < nrOrientation; i++) {
		...
		for (int yCell = 0; yCell < cellPerHeight_MAX; yCell++) {
			for (int xCell = 0; xCell < cellPerWidth_MAX; xCell++) {
				int cellPositionY = yCell * pixelPerCell;
				int cellPositionX = xCell * pixelPerCell;
				float value = cell_hog(magnitude, orientaion, 0, pixelPerCell,
						orientation_end, orientation_start, cellPositionY,
						cellPositionX);
				histogram[yCell][xCell][i] = value;
			}
		}
	}
    ...
    for (int y = 0; y < NumberBlocksY; y++)
    {
        for (int x = 0; x < NumberBlocksX; x++)
        {
            for (int i = 0; i < nrOrientation; i++)
            {
                Blockhistogram[y][x][i] = histogram[y * 2][x * 2][i];
            }
            for (int i = 0; i < nrOrientation; i++)
            {
                Blockhistogram[y][x][i+9] = histogram[y * 2][x * 2 + 1][i];
            }
            for (int i = 0; i < nrOrientation; i++)
            {
                Blockhistogram[y][x][i+2*9] = histogram[y * 2 + 1][x * 2][i];
            }
            for (int i = 0; i < nrOrientation; i++)
            {
                Blockhistogram[y][x][i+3*9] = histogram[y * 2 + 1][x * 2 + 1][i];
            }
        }
    }
    Binnorm:
    for (int y = 0; y < NumberBlocksY; y++)
        for (int x = 0; x < NumberBlocksX; x++)
            normalizeCell(Blockhistogram[y][x]);
...
void gradient(uint8_t *pictureIn, int16_t *Gx, int16_t *Gy){
	GyLoop:
    for (int y = 0; y < MAX_HEIGHT; y++){
        for (int x = 0; x < MAX_WIDTH; x++){
#pragma HLS pipeline II=2
            if(y==0 || y== MAX_HEIGHT - 1){
                ...
                int16_t tmp = (int16_t) pictureIn[(y+1)*MAX_WIDTH+x] - (int16_t) pictureIn[(y-1)*MAX_WIDTH+x];
                Gy[y*MAX_WIDTH+x] = tmp;
            }
            if(x>=MAX_WIDTH-2){
            	Gx[y*MAX_WIDTH+x+1] = 0;
            }else{
                int16_t tmp = (int16_t) pictureIn[y*MAX_WIDTH+x+2] - (int16_t) pictureIn[y*MAX_WIDTH+x];
                Gx[y*MAX_WIDTH+x+1] = tmp;
            }
...
void top_level_HLS(uint8_t *picture,float scale,obj *list){
#pragma HLS INTERFACE ap_ctrl_none port=return
#pragma HLS INTERFACE m_axi depth=32 port=list
#pragma HLS INTERFACE m_axi depth=8 port=picture
#pragma HLS INTERFACE s_axilite port=scale
    static int16_t Gx[MAX_HEIGHT*MAX_WIDTH];
    static int16_t Gy[MAX_HEIGHT*MAX_WIDTH];
    gradient(picture,Gx,Gy);
    hog_picture_old(Gx,Gy,9,scale,list);
}
