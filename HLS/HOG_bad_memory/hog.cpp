    ...
    for (int i = 0; i < nrOrientation; i++)
    {
        ...
        for (int yCell = 0; yCell < cellPerHeight_MAX; yCell++)
        {
            for (int xCell = 0; xCell < cellPerWidth_MAX; xCell++)
            {
                ...
                float value = cell_hog(magnitude, orientaion, 0, pixelPerCell, orientation_end, orientation_start, cellPositionY, cellPositionX);
...
std::vector<obj> SVM_Detection_visual_apr(Mat orginalImage,int pixelPerCell,int cellPerBlock,float scale){
    ...
    for (int y = 0; y < NumberBlocksY; y++)
        for (int x = 0; x < NumberBlocksX; x++)
            normalizeCell(Blockhistogram[y][x]);//4 Block times 9 angular bins
            ...
            for (int y = 0; y < BlocksPerWindowY; y++){
                for (int x = 0; x < BlocksPerWindowX; x++)
                {
                    for (int i = 0; i < 36; i++)
                    {
                        ...
                        ap_fixed<16, 4> BlockArrayVal =(ap_fixed<16, 4> ) Blockhistogram[y + windowY][x + windowX][i];
